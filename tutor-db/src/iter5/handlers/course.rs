...
pub async fn post_new_course(
    ...
    post_new_course_db(&app_state.db, payload.into())
        .await
        .map(|course| HttpResponse::Ok().json(course))
}
...
pub async fn update_course_details(
    ...
    update_course_details_db(&app_state.db, tutor_id, course_id, payload.into())
        .await
        .map(|course| HttpResponse::Ok().json(course))
}
...
mod tests {
    ...
    async fn test_get_course_detail_failure() {
        dotenv().ok();
        let database_url = env::var("DATABASE_URL").expect("DATABASE_URL is not set in .env file");
        let pool: PgPool = PgPool::connect(&database_url).await.unwrap(); let app_state: web::Data<AppState> = web::Data::new(AppState {
            health_check_response: "".to_string(),
            visit_count: Mutex::new(0),
            db: pool,
        });
    ...
    async fn test_update_course_success() {
        ...
        let update_course_msg = UpdateCourse {
            ...
            description: Some("This is yet another test course".into()),
            format: None,
